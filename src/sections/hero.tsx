    ...
    <section id="home" className="relative overflow-hidden">
      ...
      <Container className="relative grid grid-cols-[repeat(auto-fit,minmax(min(100%,340px),1fr))] items-center gap-[clamp(40px,6vw,68px)] pb-[clamp(56px,8vw,96px)] pt-[clamp(64px,10vw,120px)]">
        <Reveal>
          ...
          <h1 className="text-display font-bold leading-none tracking-[-0.035em]">{SITE.name}</h1>

          <p className="mt-5 max-w-[540px] text-h4 font-medium leading-[1.32] tracking-[-0.01em] text-balance">
            I build <span className="text-accent-soft">AI-powered automation platforms</span> — not
            just automated tests.
          </p>

          <p className="mt-5 max-w-[520px] text-body-lg leading-[1.7] text-fg-subtle">
            Software Engineer in Test focused on intelligent test platforms, framework architecture,
            and testing infrastructure that make software quality scalable across an engineering
            organization.
          </p>
      ...
      <div className="p-5 sm:p-6">
        ...
        <ol className="flex flex-col gap-2">
          {HERO_STEPS.map((step) => (
            <li
              key={step.n}
              className="flex items-center gap-2.5 rounded-[9px] border border-accent/20 bg-accent/[0.06] px-3 py-2.5"
            >
              <span
                aria-hidden

        ...
        <Stagger className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,160px),1fr))] gap-4">
          {BIG_METRICS.map((metric) => (
            <Reveal as="div" key={metric.label} item>
              <div className="h-full rounded-xl border border-white/[0.07] bg-surface-raised px-6 py-6 transition-colors hover:border-white/15">
                <p className="bg-gradient-to-b from-white to-fg-subtle bg-clip-text text-[length:clamp(34px,4.5vw,46px)] font-bold leading-none tracking-[-0.04em] text-transparent">
                  {metric.value}
                </p>

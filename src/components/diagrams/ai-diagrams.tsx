...
/* ==========================================================================
   ...
   Motion: the diagram *assembles itself* top → bottom when scrolled into view
   — inputs settle first, the orchestrator scales in, agents follow, the
   self-correction loop and artifacts last. Connectors pop between the nodes
   they join. This communicates the architecture's structure through the order
   of assembly, not just its final shape.
   ========================================================================== */
...
export function AgentOrchestrationDiagram() {
  return (
    ...
    >
      <Stagger stagger={STAGGER.cascade}>
        <Reveal as="div" item>
        ...
        <Reveal as="ul" item variants={fadeInScale} className="mb-2 flex flex-wrap justify-center gap-3">
        ...
        {/* Agent tier settles in as one unit. */}
        <Reveal
          as="ul"
          item
          variants={fadeInScale}
          className="mb-2 grid grid-cols-[repeat(auto-fit,minmax(min(100%,150px),1fr))] gap-3"
        >
          {AGENTS.map((agent) => (
            <li
              key={agent.tag}
              className="h-full rounded-[11px] border border-accent/[0.28] bg-accent/[0.06] px-3.5 py-4"
            >
        ...
        <Reveal as="ul" item variants={fadeInScale} className="flex flex-wrap justify-center gap-3">
